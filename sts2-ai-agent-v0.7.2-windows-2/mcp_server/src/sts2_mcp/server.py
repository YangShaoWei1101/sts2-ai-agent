...
def _normalize_game_data_collection(collection: str) -> str:
    normalized = (collection or "").strip().lower()
    if normalized not in KNOWN_GAME_DATA_COLLECTIONS:
        raise KeyError(f"Unknown game data collection: {normalized!r}")

    return normalized


def _load_game_data_collection(collection: str) -> Any:
    normalized = _normalize_game_data_collection(collection)
...
def _ensure_game_data_index(collection: str) -> dict[str, Any]:
    ...
    normalized = _normalize_game_data_collection(collection)
    with _GAME_DATA_LOCK:
        cached_index = _GAME_DATA_INDEXES.get(normalized)
        if cached_index is not None:
            return cached_index

    items = _load_game_data_collection(normalized)
    if isinstance(items, dict):
        ...
        raise TypeError(f"Unsupported data type for collection {normalized!r}: {type(items)}")

    with _GAME_DATA_LOCK:
        cached_index = _GAME_DATA_INDEXES.get(normalized)
        if cached_index is not None:
            return cached_index
        _GAME_DATA_INDEXES[normalized] = index
        return index
