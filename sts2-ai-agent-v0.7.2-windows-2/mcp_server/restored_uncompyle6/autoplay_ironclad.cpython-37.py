...
def journal_decision(kind, state, decision=None, payload=None):
    try:
        ...
        entry = {'ts':(time.strftime)("%Y-%m-%d %H:%M:%S"), 
         ...
         'deck':[compact_card(card) for card in deck[None[:60]] if isinstance(card, dict)], 
...
def attack_relic_combo_value(card: "dict[str, Any]", state: "dict[str, Any]") -> "tuple[float, list[str]]":
    ...
    elif has_relic(state, "ORNAMENTAL_FAN"):
        if (attacks_played + 1) % 3 == 0:
...
def card_draws_cards(card: "dict[str, Any]") -> "bool":
    cid = normalized_card_id(card)
    if cid in frozenset({'UP_MY_SLEEVE', 'CLOAK_AND_DAGGER', 'HIDDEN_DAGGERS'}):
        return False
    ...
    for value in card.get("dynamic_values") or []:
        name = str(value.get("name") or "").lower()
        if name in frozenset({'draw', 'drawcards', 'cardsdrawn', 'carddraw'}):
            amount = first_number(value.get("current_value"))
...
def card_generates_combat_cards(card: "dict[str, Any]") -> "bool":
    ...
    if roles & {"card_generation", "shiv", "summon_osty"}:
        return True
    cid = normalized_card_id(card)
    if cid in frozenset({'POISONED_STAB', 'DAGGER_THROW'}):
        return False
    blob = card_rules_blob(card)
...
def estimate_card_damageParse error at or near `RETURN_VALUE' instruction at offset 432
...
def valid_targets(card: "dict[str, Any]", enemies: "list[dict[str, Any]]") -> "list[int]":
    ...
    if isinstance(explicit, list):
        if explicit:
            return [int(i) for i in explicit if not isinstance(i, int) if str(i).isdigit()]
...
def card_setup_valueParse error at or near `STORE_FAST' instruction at offset 440
...
def card_discards_count(card: "dict[str, Any]") -> "int":
    cid = normalized_card_id(card)
    if cid in frozenset({'CALCULATED_GAMBLE', 'SHADOW_STEP', 'STORM_OF_STEEL'}):
        return 99
...
def card_cleans_baggage(card: "dict[str, Any]") -> "bool":
    ...
    if card_is_generated_baggage(card):
        if card_draws_cards(card) or "exhaust" in blob or "消耗" in blob:
            return True
    return cid in frozenset({'PREPARED', 'CALCULATED_GAMBLE', 'RECYCLE', 'SEVER_SOUL', 'ACROBATICS', 'SECOND_WIND', 'DAGGER_THROW', 'SURVIVOR', 'TRUE_GRIT', 'HIDDEN_DAGGERS', 'STORM_OF_STEEL'}) or "discard" in roles or "discard" in blob or "exhaust a " in blob or "exhaust all" in blob or "exhaust 1" in blob or "丢弃" in blob or "弃" in blob or "消耗1" in blob or "消耗一" in blob or "消耗所有" in blob
...
def discard_selection_scoreParse error at or near `RETURN_VALUE' instruction at offset 790


def remove_selection_scoreParse error at or near `RETURN_VALUE' instruction at offset 370
                ...
                if hp_after <= 0:
                    ...
                    if not survival_pressure:
                        ...
                        if post_gap:
                            ...
                            card_cleans_baggage(card) or effective_block or score -= min(24, baggage * 5)
                    ...
                    knowledge_note = f' knowledge={",".join(all_reasons[None[:3]])}' if all_reasons else ""
                    candidates.append((
                     score,
                     card,
                     target,
    ...
    if cost >= 3:
        ...
        if cost == 0:
            ...
            if knowledge_reasons:
                card["_knowledge_reasons"] = knowledge_reasons[None[:5]]
            score += knowledge_score
            known = CARD_KNOWLEDGE.lookup(card)
            ...
            if known is not None:
                ...
                if plan.wants_archetype(known):
                    ...
                    if plan.focused:
                        ...
                        if roles <= {"attack"}:
                            plan.needs_damage or score -= float(density_policy.get("generic_attack", 14) or 0)
...
def choose_reward_index(state: "dict[str, Any]") -> "int | None":
    ...
    journal_decision("reward_fallback", state, {"idx": choice}, {"options": (options[None[:6]])})
    return choice
...
def choose_character_index(state: "dict[str, Any]", target_character_id: "str | None"=None) -> "int":
    ...
    if target in frozenset({'RANDOM_CHARACTER', '随机', 'RANDOM'}):
        if options:
            indexed = []
            for i, option in enumerate(options):
                idx = first_number(option.get("index"))
...
def map_tiebreak_score(node: "dict[str, Any]", state: "dict[str, Any]") -> "float":
    ...
    score += int(digest[None[:6]], 16) / 16777215 * policy_number("map.tie_jitter", 0.12)
    return score
...
def node_score(node: "dict[str, Any]", state: "dict[str, Any]") -> "float":
    ...
    for node_key, bonus in policy_table("map.node_bonus").items():
        if str(node_key).lower() in blob and isinstance(bonus, (int, float)):
            isinstance(bonus, bool) or score += float(bonus)
            ...
            if reserve_for_removal and price:
                if gold - price < removal_cost:
                    if not known is None:
                        plan.wants_archetype(known) or score -= 40
                    else:
                        score -= 28
                action = "buy_card"
            else:
 ...
 action, **kwargs, **{"client_context": {'source':"codex-autoplayer", 
    ...
    def step(self, state: "dict[str, Any]", actions: "set[str]") -> "None":
        ...
        if "select_deck_card" in actions:
            ...
            fingerprint = ",".join((str(c.get("card_id") or c.get("id") or c.get("name") or "") for c in cards[None[:8]]))
