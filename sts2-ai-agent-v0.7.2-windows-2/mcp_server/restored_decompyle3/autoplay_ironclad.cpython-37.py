...
def policy_number(path: "str", default: "float"=0.0) -> "float":
    ...
    if not (isinstance(cur, bool) or isinstance(cur, (int, float))):
        return default
    return float(cur)
...
def journal_decision(kind, state, decision=None, payload=None):
    try:
        ...
        entry = {'ts':(time.strftime)("%Y-%m-%d %H:%M:%S"), 
         ...
         'potions':[compact_potion(potion) for potion in run.get("potions") or run.get("player_potions") or [] if isinstance(run, dict) else [] if isinstance(potion, dict)]}
        if payload is not None:
...
def card_energy_gain(card: "dict[str, Any]") -> "int":
    ...
    if not "gain energy" in blob:
        if "gain [e]" in blob or "获得能量" in blob:
...
def card_star_gain(card: "dict[str, Any]") -> "int":
    ...
    if amount is not None:
        return max(0, amount)
    blob = card_rules_blob(card)
    if not "gain star" in blob:
        if "获得{stars" in blob or "star_icon" in blob:
            return first_number(blob) or 1
        return 0
    ...
    if has_relic(state, "SHURIKEN"):
        ...
        if has_relic(state, "KUNAI"):
            ...
            if has_relic(state, "ORNAMENTAL_FAN"):
                if (attacks_played + 1) % 3 == 0:
                    incoming = sum((intent_damage(enemy) for enemy in combat.get("enemies") or []))
...
def card_draws_cards(card: "dict[str, Any]") -> "bool":
    cid = normalized_card_id(card)
    if cid in frozenset({'HIDDEN_DAGGERS', 'UP_MY_SLEEVE', 'CLOAK_AND_DAGGER'}):
        return False
...
def generated_card_damage(card: "dict[str, Any]") -> "int":
    ...
    if "shiv" in roles:
        return count * 4
    if not "shiv" in blob:
        if "knife" in blob or "小刀" in blob:
            return count * 4
        if cid == "UP_MY_SLEEVE":
            return count * 4
        return 0
...
def current_player(state: "dict[str, Any]") -> "dict[str, Any]":
    ...
    for player in players:
        if not not player.get("is_local"):
            if player.get("player_id") in ('1', 1):
                pass
            local_player = player
            break
...
def enemy_alive(enemy: "dict[str, Any]") -> "bool":
    if not enemy.get("is_dead"):
        if enemy.get("dead") or enemy.get("escaped"):
            return False
...
def intent_damage(enemy: "dict[str, Any]") -> "int":
    ...
    if isinstance(intents, list):
        ...
        if total:
            return total
        candidates = [enemy.get("total_damage"),
...
def enemy_status_pressure(enemy: "dict[str, Any]") -> "int":
    ...
    for intent in enemy.get("intents") or []:
        ...
        if not not "debuff" in intent_type:
            if "status" in intent_type:
                pass
            pressure += 2
...
def enemy_threat_score(enemy: "dict[str, Any]") -> "float":
    ...
    if any((token in enemy_id for token in ('KIN_FOLLOWER', 'CULTIST', 'HEALER', 'PRIEST'))):
        score += 14
    if any((token in enemy_id for token in ('STRANGLER', 'SENTRY', 'JAXFRUIT'))):
        score += 8
...
def enemy_target_score(enemy, enemies, *, damage, lethal):
    ...
    minion_count = sum((1 for candidate in alive))
...
def estimate_card_damage(card: "dict[str, Any]") -> "int":
    ...
    if card_type != "attack" and "deal" not in blob:
        if "deals" not in blob:
            return 0
        if not "bash" in cid:
            if "bash" in name or "重锤" in name:
                return 8
...
def estimate_card_block(card: "dict[str, Any]") -> "int":
    ...
    if card_type == "attack":
        return 0
    if not "defend" in cid:
        if "defend" in name or "防御" in name:
            return 5
...
def valid_targets(card: "dict[str, Any]", enemies: "list[dict[str, Any]]") -> "list[int]":
    ...
    if isinstance(explicit, list):
        if explicit:
            return [int(i) for i in explicit if not isinstance(i, int) if str(i).isdigit() if isinstance(i, int) if str(i).isdigit()]
...
def card_setup_value(card, state, cards):
    ...
    incoming = sum((intent_damage(enemy) for enemy in enemies))
    current_block = first_number(current_player(state).get("block")) or first_number(combat.get("block")) or 0
    energy = player_energy(state)
    energy_after_card = max(0, energy - cost + card_energy_gain(card))
    ...
    has_followup_attack = any((combat_card_damage(other, state) > 0 and card_cost(other) <= energy_after_card for other in cards))
    has_followup_block = any((not same_card_instance(other, card) and other.get("playable") is not False and estimate_card_block(other) + attack_relic_block(other, state) > 0 and card_cost(other) <= energy_after_card for other in cards))
    affordable_missing_star = any((not same_card_instance(other, card) and card_star_cost(other) > stars and card_star_cost(other) <= stars + card_resource_gain(card) and (combat_card_damage(other, state) > 0 or estimate_card_block(other) > 0 or known_card_roles(other) & {"power_scaling", "debuff", "summon_osty"}) for other in cards))
    ...
    is_power_setup = "power_scaling" in roles or known_card_type(card) == "power" or "能力" in blob
    is_debuff_setup = roles & {"vulnerable", "weak", "debuff", "doom"} or any((token in blob for token in ('vulnerable',
                                                                                                           'weak',
                                                                                                           'doom',
                                                                                                           '易伤',
                                                                                                           '虚弱',
                                                                                                           '灾厄')))
    if not cid in frozenset({'SUMMON_FORTH', 'BODYGUARD', 'NECRO_MASTERY'}):
        is_summon_setup = any((token in blob for token in ('summon', '召唤'))) and cid != "UNLEASH"
        if draws_cards:
            draw_bonus = 22 if cost <= 1 else 12
            if baggage >= 3 and not card_is_discard_now(card):
                if not card_is_generated_baggage(card):
                    draw_bonus -= min(18, baggage * 4)
                    reasons.append("clogged-draw")
                if baggage:
                    if card_cleans_baggage(card):
                        draw_bonus += min(14, baggage * 4)
                    ...
                    if generates_cards:
                        ...
                        if "shiv" in roles or "shiv" in blob or "小刀" in blob:
                            gen_bonus += 8
                        if incoming <= current_block + block + 8 or cards_played == 0:
                            gen_bonus += 6
                        score += gen_bonus
                        reasons.append("setup-generate")
                    if grants_resource:
                        resource_bonus = 18 + min(12, card_resource_gain(card) * 4)
                        if affordable_missing_star:
                            resource_bonus += 18
                            reasons.append("enables-star-card")
                        score += resource_bonus
                        reasons.append("setup-energy")
                    if not is_power_setup or incoming <= current_block + block + 8 or plan.needs_scaling or cards_played == 0:
                        score += 30
                    ...
                    if is_debuff_setup:
                        ...
                        if has_followup_attack:
                            debuff_bonus += 14
                        if incoming > current_block:
                            if "weak" in roles or "weak" in blob or "虚弱" in blob:
                                debuff_bonus += 10
                            if "doom" in roles or "doom" in blob or "灾厄" in blob:
                                debuff_bonus += 10
                            score += debuff_bonus
                        ...
                        if is_summon_setup:
                            summon_bonus = 26
                            if cid == "BODYGUARD":
                                if any((normalized_card_id(c) == "UNLEASH" for c in cards)):
                                    summon_bonus += 14
                                if incoming <= current_block + block + 12 or cards_played == 0:
                                    summon_bonus += 8
                                score += summon_bonus
                                reasons.append("setup-summon")
                            if cid == "UNLEASH":
                                if any((normalized_card_id(other) == "BODYGUARD" and not same_card_instance(other, card) for other in cards)):
                                    score -= 44
                                    reasons.append("wait-bodyguard")
                                else:
                                    score += 12
                                    reasons.append("osty-payoff")
                        ...
                        if damage and has_followup_attack:
                            if any((token in blob for token in ('vulnerable', '易伤'))):
                                score += 12
                            if cost >= energy:
                                if damage == 0:
                                    if block == 0:
                                        if incoming > current_block + 12:
                                            if has_followup_block:
                                                score -= 26
...
def card_is_generated_baggage(card: "dict[str, Any]") -> "bool":
    ...
    roles = known_card_roles(card)
    blob = text_blob(card)
    return cid in STATUS_BAGGAGE_IDS or cid.endswith("_STATUS") or cid.endswith("_CURSE") or cid.endswith("_WOUND") or card_type in frozenset({'status', 'quest', 'curse'}) or "curse" in rarity or "not_for_drafting" in roles or "status" in blob or "curse" in blob
...
def hand_playable_payload_count(cards: "list[dict[str, Any]]", state: "dict[str, Any]") -> "int":
    count = 0
    for card in cards:
        if not card.get("playable") is False:
            if card_is_generated_baggage(card):
                continue
            if not not combat_card_damage(card, state):
                if not not estimate_card_block(card):
...
def card_discards_count(card: "dict[str, Any]") -> "int":
    cid = normalized_card_id(card)
    if cid in frozenset({'STORM_OF_STEEL', 'CALCULATED_GAMBLE', 'SHADOW_STEP'}):
        return 99
    ...
    if "discard your hand" in blob or "丢弃你的手牌" in blob:
        return 99
    if not "discard" in blob:
        if "丢弃" in blob or "弃" in blob:
            nums = [int(x) for x in re.findall("\\d+", blob)]
...
def card_cleans_baggage(card: "dict[str, Any]") -> "bool":
    ...
    if card_is_generated_baggage(card):
        if card_draws_cards(card) or "exhaust" in blob or "消耗" in blob:
            return True
        return cid in frozenset({'RECYCLE', 'CALCULATED_GAMBLE', 'DAGGER_THROW', 'STORM_OF_STEEL', 'SURVIVOR', 'HIDDEN_DAGGERS', 'SEVER_SOUL', 'SECOND_WIND', 'TRUE_GRIT', 'ACROBATICS', 'PREPARED'}) or "discard" in roles or "discard" in blob or "exhaust a " in blob or "exhaust all" in blob or "exhaust 1" in blob or "丢弃" in blob or "弃" in blob or "消耗1" in blob or "消耗一" in blob or "消耗所有" in blob
...
def discard_selection_score(card: "dict[str, Any]", state: "dict[str, Any]") -> "float":
    ...
    cost = card_cost(card)
    combat = state.get("combat") or {}
    incoming = sum((intent_damage(e) for e in combat.get("enemies") or []))
    current_block = first_number(current_player(state).get("block")) or first_number(combat.get("block")) or 0
    score = 0.0
    ...
    if card_is_generated_baggage(card):
        score += 230
    if card.get("playable") is False:
        if not card_has_sly(card):
            score += 70
        if cid in SILENT_DISCARD_SCALING_IDS:
            score -= 120
        ...
        if card_is_discard_now(card):
            ...
            if "power_scaling" in roles:
                score -= 70
            if "draw" in roles:
                if cid not in SILENT_SLY_RESOURCE_IDS:
                    score -= 18
        if "energy" in roles or "star_resource" in roles:
            if cid not in SILENT_SLY_RESOURCE_IDS:
                score -= 25
            need_block = max(0, incoming - current_block)
            if block:
                score -= min(block, need_block) * 2.5 + max(0, block - need_block) * 0.4
    if dmg:
        score -= min(dmg, 24) * 1.2
        enemies = combat.get("enemies") or []
        if any((enemy_alive(enemy) and dmg >= enemy_hp(enemy) for enemy in enemies)):
            score -= 45
        blob = text_blob(card)
        if any((key in blob for key in ('strike', 'defend', 'strike_silent', 'defend_silent'))):
            score += 48 if "strike" in blob else 36
        if card_type == "attack":
            if incoming > current_block:
                score += 8
        if cost >= 3:
            if cid not in SILENT_DISCARD_SCALING_IDS | SILENT_DISCARD_ENGINE_IDS:
                score += 18
    if cost == 0 and not card_has_sly(card):
        if not card_is_generated_baggage(card):
            score -= 8
...
def remove_selection_score(card: "dict[str, Any]", state: "dict[str, Any]") -> "float":
    ...
    if not card.get("selected"):
        if card.get("is_selected") or card.get("chosen") or card.get("disabled"):
            return -999.0
        if card_type in frozenset({'status', 'quest', 'curse'}) or "curse" in rarity or "not_for_drafting" in roles:
            score += 260
        if cid in frozenset({'STRIKE_DEFECT', 'STRIKE_SILENT', 'STRIKE_NECROBINDER', 'STRIKE', 'STRIKE_REGENT', 'STRIKE_IRONCLAD'}):
            score += 130
            if plan.needs_damage:
                score -= 25
            if cid in frozenset({'DEFEND_NECROBINDER', 'DEFEND_IRONCLAD', 'DEFEND', 'DEFEND_REGENT', 'DEFEND_DEFECT', 'DEFEND_SILENT'}):
                score += 115
                if plan.needs_block:
                    score -= 30
                if card.get("playable") is False:
                ...
                if card.get("upgraded") or card.get("is_upgraded"):
                    score -= 35
            if str(card.get("rarity") or "").lower() in frozenset({'uncommon', 'rare'}):
                score -= 18
        return score


def best_discard_target_score(cards, state, *, exclude=None):
    scores = [discard_selection_score(card, state) for card in cards if not exclude is None if not same_card_instance(card, exclude)]
    if scores:
        return max(scores)
    return -999.0
...
def discard_combat_modifier(card, state, playable, *, incoming, current_block):
    ...
    if card_is_discard_now(card):
        ...
        elif cid == "DAGGER_THROW":
            score += 7
        elif cid == "HIDDEN_DAGGERS":
        ...
        if cid in SILENT_WHOLE_HAND_DISCARD_IDS:
            ...
            for other in hand:
                if same_card_instance(other, card):
                    ...
                    if other_id in SILENT_DISCARD_ENGINE_IDS | SILENT_DISCARD_SCALING_IDS or other_value >= 12:
                        valuable_cards += 1
                if other_id == "MEMENTO_MORI":
                    if other.get("playable") is not False:
...
def followup_combo_value(card, state, cards, target, *, incoming, current_block, energy, stars):
    ...
    choices = []
    for other in cards:
        if not same_card_instance(other, card):
            if other.get("playable") is False:
                continue
            if card_is_generated_baggage(other):
                if not not card_cleans_baggage(other):
                    continue
                ...
                if other_damage:
                    ...
                    if other_target is not None:
                        if not 0 <= other_target< len(enemies):
    ...
    elif card_draws_cards(card) or card_is_discard_now(card):
        reasons.append("unlocks-card-flow")
    elif card_generates_combat_cards(card):
...
def choose_combat_action(state: "dict[str, Any]") -> "tuple[str, dict[str, int | None], str]":
    ...
    if not cards:
        return ("end_turn", {}, "no playable cards")
    ...
    if hp:
        ...
        energy = player_energy(state)
        cards_played = played_this_turn(state)
        total_attack_damage = sum((combat_card_damage(c, state) for c in cards))
        alive_hp = sum((enemy_hp(e) for e in enemies))
        ...
        for card in cards:
            ...
            if effective_block:
                ...
                if survival_pressure:
                    block_weight = 4.8 if (not can_sweep_lethal) else 2.4
            ...
            if setup_score > 0:
                ...
                if survival_pressure:
                    if not can_sweep_lethal:
                        if effective_block:
                            score += 10
                        elif setup_score > 0 and can_cover_with_hand and cost < energy:
                            score -= 4
                ...
                if preventable_gap and not can_sweep_lethal:
                    ...
                    if survival_pressure or hp_ratio < 0.55 or preventable_gap >= 12:
            ...
            if baggage:
                if card_cleans_baggage(card):
                    ...
                    if baggage >= 3:
                        if card_draws_cards(card):
                            if not not card_cleans_baggage(card):
        ...
        if best_score <= damage_floor:
            if not (best_block and damage_gap):
    ...
    if known is not None:
        ...
        if plan.needs_damage:
            ...
            if plan.needs_block:
                ...
                if plan.focused and known.archetypes:
                    ...
                    if plan.combat_ready and plan.card_count >= 18:
                        ...
                        if plan.card_count >= 22 and not matching_plan:
                            if not utility:
                                if not fills_need:
...
def reward_options(state: "dict[str, Any]") -> "list[dict[str, Any]]":
    ...
    for key in ('cards', 'card_options', 'options'):
        ...
        if isinstance(val, list):
            if any((item.get("card_id") or item.get("id") for item in val)):
                return val
...
def choose_claim_reward_index(state: "dict[str, Any]") -> "int | None":
    ...
    for (i, reward) in enumerate(rewards):
        if not reward.get("claimed"):
            if not reward.get("disabled"):
                if not reward.get("is_locked"):
                    if reward.get("claimable") is False:
                        continue
                    else:
                        ...
                        if "potion" in blob or "鑽\ue21b按" in blob:
                            score += 15
                        if "gold" in blob or "閲戝竵" in blob:
                            score += 18
                    if score > best[0]:
                        best = (
...
def choose_reward_index(state: "dict[str, Any]") -> "int | None":
    ...
    for (i, option) in enumerate(options):
        ...
        if not "skip" in blob:
            if "跳过" in blob:
                continue
            if not not "card" in blob:
                if not not option.get("card_id"):
                    if option.get("id"):
                        pass
            ...
            if not reward.get("disabled"):
                if not reward.get("is_locked"):
                    if reward.get("claimable") is False:
                        ...
                        score = 0.0
                    if not not "card" in blob:
                        if reward.get("card_id") or reward.get("id"):
                            score += 45
                        if "relic" in blob or reward.get("relic_id"):
                            score += 42
                        if "potion" in blob or reward.get("potion_id"):
                            score += 15
                        if "gold" in blob:
                            score += 18
...
def map_options(state: "dict[str, Any]") -> "list[dict[str, Any]]":
    ...
    def usable(nodes: "list[dict[str, Any]]") -> "list[dict[str, Any]]":
        return [x for x in nodes if not x.get("is_unavailable") if not x.get("locked") if not x.get("disabled") if str(x.get("state") or "").lower() not in frozenset({'traveled', 'untravelable', 'visited'}) if not x.get("visited") if x.get("is_available", True) is not False if x.get("is_unavailable") if x.get("locked") if x.get("disabled") if str(x.get("state") or "").lower() not in frozenset({'traveled', 'untravelable', 'visited'}) if x.get("visited") if x.get("is_available", True) is not False]
...
def map_coord(node: "dict[str, Any]") -> "tuple[int | None, int | None]":
    ...
    return (
     int(row) if row is not None else None, int(col) if col is not None else None)
    ...
    if hp:
        ...
        def penalty_for(candidate, distance):
            ...
            if "elite" in blob:
                ...
                if any((k in blob for k in ('monster', 'combat', 'enemy', 'fight',
                                            'normal'))):
            ...
            if any((k in blob for k in ('event', 'unknown', '?'))):
...
def node_score(node: "dict[str, Any]", state: "dict[str, Any]") -> "float":
    ...
    if hp:
        ...
        non_basic = sum((1 for card in deck))
        plan = deck_plan(state)
        ...
        if any((k in blob for k in ('elite', '精英'))):
            ...
            if plan.needs_scaling and ratio > 0.86:
                if non_basic >= 5:
                    elite_ready = True
                score += 26 if elite_ready else -42
                if elite_ready:
                    score += policy_number("map.combat_ready_elite_bonus", 0.0)
                if not any((k in blob for k in ('monster', 'combat', 'enemy', '普通',
                                                '敌'))) or plan.needs_damage or plan.needs_block or plan.needs_draw:
                ...
                if any((k in blob for k in ('rest', 'campfire', '篝火', '休息'))):
                    ...
                    if plan.combat_ready:
                        ...
                        if any((k in blob for k in ('shop', '商店'))):
                            ...
                            if any((k in blob for k in ('event', 'unknown', '?', '未知',
                                                        '事件'))):
    ...
    if hp:
        ...
        for (i, opt) in enumerate(opts):
            ...
            if any((k in blob for k in ('heal', '恢复', '治疗'))):
                score += 20 if ratio < 0.55 else 8 if ratio < 0.8 else -6
            if any((k in blob for k in ('fight', 'combat', 'battle', 'enter combat'))) or any((k in text_key for k in ('fight',
                                                                                                                       'combat',
                                                                                                                       'battle'))):
                score += 8 if (ratio > 0.72) and (plan.combat_ready) else (-34 if ratio > 0.7 else -8)
            else:
                if any((k in blob for k in ('relic', '遗物', 'card', '卡牌', 'upgrade',
                                            '升级', 'max hp', '最大生命'))):
                    score += 10
                if any((k in blob for k in ('upgrade', '升级', 'smith', '锻造'))):
                    score += 10 if choose_upgrade_target(state) is not None else -4
                if any((k in blob for k in ('remove', '移除', 'forget', '遗忘', 'purge',
                                            '删除'))):
                ...
                if any((k in blob for k in ('card', '卡牌', 'choose', '选择'))):
                    if plan.needs_damage or plan.needs_block or plan.needs_draw:
                        ...
                        if plan.focused:
                            score -= 4
                    if any((k in blob for k in ('lose hp', '失去', 'damage', '受伤', '诅咒',
                                                'curse'))):
                        penalty = 22 if ratio < 0.6 else 8
                        if any((k in blob for k in ('relic', '遗物', 'remove', '移除',
                                                    'upgrade', '升级'))):
                            if ratio > 0.72:
                                penalty -= 5
                            score -= penalty
                    ...
                    if is_decipher:
                        ...
                        if current_max_hp is not None:
                            ...
                            if is_give_up:
                                score += 28
                                if not ratio < 0.9:
                                    if not current_max_hp is not None or current_max_hp <= 72:
                                        score += 32
                is_rest = event_id == "DENSE_VEGETATION" and option_id == "REST" or "rest" in text_key
                ...
                if plan.needs_damage:
                    ...
                    if plan.needs_block:
                        if "block" in known.roles:
                            score += 6
                        if plan.needs_draw:
                            if not not "draw" in known.roles:
                                if "energy" in known.roles:
                                    score += 5
                                if plan.needs_scaling:
                                    if "power_scaling" in known.roles:
                                        score += 8
        ...
        if selection_cards:
            ...
            for (i, card) in enumerate(source_cards):
                ...
                if effective_idx in avoid:
                    ...
                    if any((k in blob_card for k in ('draw', 'energy', 'vulnerable',
                                                     'weak', 'power', 'strength'))):
                        score += 8
                    cost = card_cost(card)
                                ...
                                if not 0 <= target< len(enemies):
                                    ...
                                    if not not danger:
                                        if incoming <= 0:
                                            if kill_score < 36:
                                                score -= 45
                                            if any((k in potion_id for k in ('WEAK',
                                                                             'VULNERABLE',
                                                                             'SHACKLING'))):
...
def shop_synergy_buy_score(best: "tuple[float, Any, Any, dict[str, Any]]", state: "dict[str, Any]") -> "float":
    ...
    if not (action != "buy_card" or isinstance(item, dict)):
        return 0.0
    plan = deck_plan(state)
...
def choose_shop_action(state: "dict[str, Any]") -> "tuple[str, dict[str, int], str]":
    ...
    if "open_shop_inventory" in actions:
        return ("open_shop_inventory", {}, "open shop inventory")
    run = state.get("run") or {}
    gold = first_number(run.get("gold") or run.get("player_gold")) or 0
    ...
    for (i, item) in enumerate(items):
        ...
        if reserve_for_removal and price:
            if gold - price < removal_cost:
                if known is None or not plan.wants_archetype(known):
                    score -= 40
                else:
                    score -= 28
                action = "buy_card"
        if not not item.get("relic_id"):
            if "relic" in blob or "遗物" in blob:
                score = 28 - price / 40
            ...
            if reserve_for_removal:
                if price:
                    if gold - price < removal_cost:
                        score -= 18
                    action = "buy_relic"
            if not not item.get("potion_id"):
                if "potion" in blob or "药水" in blob:
                    score = 8 - price / 50
    ...
    if "remove_card_at_shop" in actions and gold >= removal_cost:
        ...
        if plan.wants_removal and can_buy_then_remove:
            if synergy_buy_score >= max(35.0, removal_score):
                ...
                if plan.wants_removal and removal_first:
                    ...
                    if "remove_card_at_shop" in actions and gold >= removal_cost:
                        if plan.wants_removal:
                            if best[0] < removal_score or gold < reserve:
                                journal_decision("shop_remove", state, {'action':"remove_card_at_shop", 
 ...
 action, **kwargs, **{"client_context": {'source':"codex-autoplayer", 
        ...
        while self.steps < max_steps:
            ...
            if state.get("game_over") or screen == "GAME_OVER":
                ...
                if game_over.get("is_victory"):
                    return {'status':"victory",  'steps':self.steps,  'attempts':self.attempts,  'state':state}
                if "return_to_main_menu" in as_actions(state):
            ...
            if not not actions:
                ...
                if screen == "UNKNOWN" and not state.get("in_combat"):
                    if self.unknown_waits >= 2:
    ...
    def step(self, state: "dict[str, Any]", actions: "set[str]") -> "None":
        ...
        if any((a in actions for a in ('open_shop_inventory', 'buy_card', 'buy_relic',
                                       'buy_potion', 'remove_card_at_shop', 'close_shop_inventory'))):
            ...
            kwargs = {}
            spec = next((a for a in action_specs(self.client.get_available_actions())), {})
