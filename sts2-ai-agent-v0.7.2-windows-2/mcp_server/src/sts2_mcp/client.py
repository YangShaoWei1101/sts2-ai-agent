...
def _json_loads_response(response_body: bytes, *, status_code: int, path: str | None = None) -> Any:
    try:
        ...
        raise Sts2ApiError(
            ...
            message="Server returned a response that was not UTF-8 encoded.",
            details={"path": path},
        ) from exc
...
class Sts2Client:
    ...
    def get_available_actions(self) -> list[dict[str, Any]]:
        ...
        if not isinstance(actions, list):
            raise Sts2ApiError(
                status_code=200,
                code="invalid_response",
                message="Server response did not contain an actions list.",
                details=payload,
            )
    ...
    def get_game_data_collection(self, collection: str) -> Any:
        encoded_collection = parse.quote(collection.strip(), safe="")
        return self._request("GET", f"/data/{encoded_collection}", expect_object_data=False)
        ...
        while True:
            ...
            try:
                ...
                if time.monotonic() >= deadline:
                    return None
                time.sleep(min(_EVENT_RECONNECT_BACKOFF, max(0.0, deadline - time.monotonic())))
    ...
    def _request(
        ...
        *,
        ...
        for attempt in range(attempts):
            ...
            try:
                with request.urlopen(http_request, timeout=timeout) as response:
                    return self._decode_success(response.read(), expect_object_data=expect_object_data, path=path)
            except error.HTTPError as exc:
                last_error = self._build_api_error(exc.code, exc.read(), path=path)
                ...
                last_error = Sts2ApiError(
                    ...
                    code="connection_error",
                    message=(
                        f"Timed out while calling STS2 mod at {self._base_url}. "
                        "Ensure the game is responsive and the mod is loaded."
                    ),
                    details={"reason": str(exc), "path": path, "timeout_seconds": timeout},
                    retryable=True,
                )
                if attempt >= self._max_retries:
                    raise last_error
    ...
    @staticmethod
    def _decode_success(response_body: bytes, *, expect_object_data: bool = True, path: str | None = None) -> Any:
        payload = _json_loads_response(response_body, status_code=200, path=path)
        if not isinstance(payload, dict):
            raise Sts2ApiError(
                status_code=200,
                code="invalid_response",
                message="Server response was JSON but not an object.",
    ...
    @staticmethod
    def _build_api_error(status_code: int, response_body: bytes, *, path: str | None = None) -> Sts2ApiError:
        try:
            payload = _json_loads_response(response_body, status_code=status_code, path=path)
        except Sts2ApiError as exc:
            return exc
        ...
        if not isinstance(payload, dict):
            return Sts2ApiError(
                status_code=status_code,
                code="invalid_response",
                message="Server error response was JSON but not an object.",
                details={"path": path, "payload_type": type(payload).__name__},
            )
