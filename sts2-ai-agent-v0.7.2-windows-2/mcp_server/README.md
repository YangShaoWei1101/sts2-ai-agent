...
Windows PowerShell 的默认代码页可能会把 UTF-8 日志显示成乱码。运行自动脚本时优先使用这些入口，它们会先固定控制台、Python I/O 和日志读取编码：

```powershell
cd "<repo-root>/mcp_server"
.\start_autoplay_utf8.cmd
```

查看日志：

```powershell
.\tail_autoplay_utf8.cmd -Kind log -Tail 200
.\tail_autoplay_utf8.cmd -Kind journal -Tail 80
.\tail_autoplay_utf8.cmd -Kind log -Tail 200 -Wait
```

停止主程序：

```powershell
.\stop_autoplay_utf8.cmd
```
